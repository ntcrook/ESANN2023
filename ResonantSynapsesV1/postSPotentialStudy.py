...
isiRelevanceThresh = 0.1                                # Threshold on gBeta to determin if isi is relevant
...
I1 = 20
I2 = 20
tsstsA = [10, 10 + I1 , 10 + I1 + I2]
tsstsB = [10, 10 + I2, 10 + I2 + I1]
tsstsInit = [0,0,0]
...
def calcISI(clusterId, synapseIndex, t,lastUpdate):
    ...
    if lastUpdate > 0:
        isi = t - lastUpdate
        scaledGBeta = gBetaScale * gBeta(isi, lowISI_s, highISI_s, getBetaParam(0, 1, synapseIndex),
...
synNo1 = 1
for i in range(0,2):
    S.clusterId['synapse_number == i'] = i
S.delay[0] = 0*ms
S.delay[1] = 0*ms
...
def updateTSSTs(I1, I2):
    global tsstsA, tsstsB, q, r, meanBeta, varBeta
    I1 = I1
    I2 = I2
    tsstsA = [10, 10 + I1, 10 + I1 + I2]
    tsstsB = [10, 10 + I2, 10 + I2 + I1]
...
def main():
    ...
    print(f'tsstsA = {tsstsA}')
    print(f'tsstsB = {tsstsB}')
    syn0Delay = tsstsA[2] - tsstsA[1] - 10  # update delay to the first transmission arrives 10ms before the second
    S.delay[0] = syn0Delay*ms
    print(f'S.delay[0] = {syn0Delay}')
    # Run the model
    inp.set_spikes([0,0,0], tsstsA * ms)  #+ defaultclock.t)
    run(200*ms)
    inp.set_spikes([0,0,0], tsstsB * ms + 200*ms)
    run(200*ms)
    # Plot the results
    plotGBeta(0)
    figure(figsize=(5, 3))
